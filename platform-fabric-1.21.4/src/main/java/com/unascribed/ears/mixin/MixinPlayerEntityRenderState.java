...
public class MixinPlayerEntityRenderState implements EarsPlayerRenderState {
	...
	public void ears$update(AbstractClientPlayerEntity acpe, float delta) {
		ears$capeX = MathHelper.lerp(delta, acpe.prevCapeX, acpe.capeX);
		ears$capeY = MathHelper.lerp(delta, acpe.prevCapeY, acpe.capeY);
		ears$capeZ = MathHelper.lerp(delta, acpe.prevCapeZ, acpe.capeZ);
		ears$horizontalSpeed = MathHelper.lerp(delta, acpe.lastDistanceMoved, acpe.distanceMoved);
		ears$stride = MathHelper.lerp(delta, acpe.prevStrideDistance, acpe.strideDistance);
