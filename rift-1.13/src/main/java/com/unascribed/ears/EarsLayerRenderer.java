...
public class EarsLayerRenderer implements LayerRenderer<AbstractClientPlayer>, EarsRenderDelegate {
	...
	public void render(AbstractClientPlayer entity, final float limbAngle, final float limbDistance,
		...
		if (tex instanceof EarsFeaturesHolder && !entity.isInvisible()) {
			...
			EarsCommon.render(((EarsFeaturesHolder)tex).getEarsFeatures(), this, limbDistance, ((AccessorModelPlayer)render.getMainModel()).ears$isSmallArms());
	...
	public void renderLeftArm(AbstractClientPlayer entity) {
		...
		if (tex instanceof EarsFeaturesHolder && !entity.isInvisible()) {
			...
			GlStateManager.enableBlend();
			GlStateManager.blendFunc(SourceFactor.SRC_ALPHA, DestFactor.ONE_MINUS_SRC_ALPHA);
			EarsCommon.render(((EarsFeaturesHolder)tex).getEarsFeatures(), this, 0, ((AccessorModelPlayer)render.getMainModel()).ears$isSmallArms());
			GlStateManager.disableBlend();
			GlStateManager.disableRescaleNormal();
			GlStateManager.disableCull();
		}
