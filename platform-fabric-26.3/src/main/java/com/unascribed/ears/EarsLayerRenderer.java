	...
	private final IndirectEarsRenderDelegate<PoseStack, SubmitNodeCollector, RenderType, AvatarRenderState, ModelPart> delegate = new IndirectEarsRenderDelegate<>() {
		...
		protected void doBindBuiltin(TexSource src) {
			...
			if (src.isGlint()) {
				...
				vc = RenderTypes.trimmedArmorGlint();
			} else if (canBind(src)) {
				...
				try {
					...
					aafr.ears$renderArmor(matrices, new SubmitNodeCollector() {
						...
						@Override
						public void submitItem(PoseStack poseStack, ItemDisplayContext displayContext, int lightCoords, int overlayCoords, int outlineColor, int[] tintLayers, ItemQuads quads, FoilType foilType) {
							
						}

						@Override
						public <S> void submitModel(Model<? super S> arg0, S arg1, PoseStack arg2, RenderType arg3, int arg4, int arg5, int arg6, @org.jspecify.annotations.Nullable UvMapping arg7, int arg8) {
							vc = arg3;
						}
						...
						@Override
						public void submitNameTag(PoseStack arg0, @org.jspecify.annotations.Nullable Vec3 arg1, int arg2, Component arg3, boolean arg4, int arg5, CameraRenderState arg6) {
						}
		...
		protected void addVertex(float x, float y, int z, float r, float g, float b, float a, float u, float v, float nX, float nY, float nZ) {
			...
			final int packedLight = emissive ? LightCoordsUtil.pack(15, 15) : light;
